...
class BalrogFit(object):
    ...
    def _set_plugins(self):
        ...
        for active_interval in self._trigger_info["active_intervals"]:

            active_time_start = active_interval["active_time_start"]
            ...
            for det in self._trigger_info["use_dets"]:
                ...
                ogip_like = OGIPLike(
                    f"grb{det}i{interval_id}",
                    observation=os.path.join(
                        pha_output_dir, f"{self._trigger_name}_{interval_id}_{det}.pha"
                    ),
                    background=os.path.join(
                        pha_output_dir,
                        f"{self._trigger_name}_{interval_id}_{det}_bak.pha",
                    ),
                    response=rsp,
                    spectrum_number=1,
                )
