...
parser.add_argument(
    ...
    "--bkg_fit_result",
    type=str,
    help="Path to the background fit result that contains the background model from a previous fit",
)
...
from gbmbkgpy.modeling.functions import BkgModelFunction
from gbmbkgpy.modeling.source import GlobalSource
import h5py

# TODO: add checks for dates, data_type, detectors and echans
with h5py.File(args.bkg_fit_result, "r") as f:
...
# Build the bkg model source from the previous fit
bkg_model = BkgModelFunction("norm_bkg_model")
bkg_model.set_detector_mask(model_generator.data.detector_mask)
bkg_model.set_echan_mask(model_generator.data.echan_mask)
bkg_model.set_time_bins(model_generator.data.time_bins)

bkg_model.set_source_counts(model_counts)
bkg_model.set_source_time_bins(time_bins)

bkg_model_source = GlobalSource("bkg_model", bkg_model)

model_generator._source_list.append(bkg_model_source)

model_generator._instantiate_model(model_generator.config)

model_generator._model.set_parameter_priors(config["priors"]["bkg_model"])
