...
from gbm_bkg_pipe.utils.stats.xlogy import xlogyv
from gbm_bkg_pipe.utils.stats.z_bi import z_bi_vectorized
from gbm_bkg_pipe.utils.stats.size_one_or_n import size_one_or_n

# Implements the formulae of Vianello 2018 for computing the significance in counting experiments with gaussian errors.
# From https://github.com/giacomov/gv_significance
...
def significance(n, b, sigma):
    ...
    B0_mle = 0.5 * (
        b_
        - sigma_ ** 2
        + sqrt(b_ ** 2 - 2 * b_ * sigma_ ** 2 + 4 * n_ * sigma_ ** 2 + sigma_ ** 4)
    )
